...
const PLATFORMS = [
  ...
  { id: 'reddit', labelKey: 'platform_reddit', labelFallback: 'Reddit', icon: '🟠' },
...
function detectPlatform(url: string): string {
  ...
  if (/reddit\.com|redd\.it|v\.redd\.it/i.test(url)) return 'reddit'
...
export default function App() {
  ...
  const fetchAdminUsers = async () => {
    try {
      const r = await axios.get(`${API_BASE}/api/auth/admin/users`, { headers: getAuthHeaders(), params: { search: adminUserSearch, tier: adminUserTier } });
  ...
  return (
    <div className={`min-h-screen ${isDark ? 'dark-mode bg-dark-bg text-white' : 'light-mode bg-light-bg text-light-text'}`}>
      {/* 横屏保护遮罩 - PWA兜底 */}
