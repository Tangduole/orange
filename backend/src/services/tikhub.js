...
async function parseBilibili(url, taskId, onProgress) {
  ...
  console.log(`[TikHub] Parsing Bilibili: ${bvid}`);
  if (onProgress) onProgress(10);

  // 获取视频信息
  const info = await tikhubRequest(`/api/v1/bilibili/web/fetch_video_info?bvid=${bvid}`, API_KEY_XHS);
  if (onProgress) onProgress(20);
  ...
  // 获取播放地址
  const playUrl = await tikhubRequest(`/api/v1/bilibili/web/fetch_video_playurl?bvid=${bvid}&cid=${info.cid || ''}`, API_KEY_XHS);
  if (onProgress) onProgress(30);

  // 找到最好的视频流
  let videoUrl = '';
  const durl = playUrl.durl || [];
  if (durl.length > 0) {
    videoUrl = durl[0].url || '';
  }

  if (!videoUrl) {
    throw new Error('No download URL found from TikHub Bilibili');
  }

  console.log(`[TikHub] Found Bilibili video: ${title}`);
  ...
  if (pic) {
    const thumbPath = path.join(DOWNLOAD_DIR, `${taskId}_thumb.jpg`);
    try {
      await downloadFile(pic.startsWith('http') ? pic : `https:${pic}`, thumbPath);
      thumbnailUrl = `/download/${taskId}_thumb.jpg`;
    } catch {}
  }
...
module.exports = { parseYouTube, parseXiaohongshu, parseBilibili, tikhubRequest };
